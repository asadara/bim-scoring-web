...
export async function exportPmpArea15Workbook(input: PmpArea15WorkbookInput): Promise<string> {
  const overviewRows = [
    ...
    ["PMP15 optional input score", input.summary.scoring_adjustment?.pmp_score_0_5 ?? "N/A"],
    ["PMP15 optional bonus", input.summary.scoring_adjustment?.bonus_score_100 ?? 0],
    ["Adjusted BIM score", input.summary.scoring_adjustment?.adjusted_total_score_100 ?? input.summary.total_bim_score_100 ?? "N/A"],
    ["PMP15 blocks approval", input.summary.scoring_adjustment?.blocks_approval ? "YES" : "NO"],
    ["Export ready", input.summary.export_ready ? "YES" : "NO"],
    ["Hold point readout", input.summary.hold_point_ready ? "YES" : "NO"],

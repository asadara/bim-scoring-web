...
const API_BASE = String(window.__BCL_API_BASE_URL__ || "").replace(/\/+$/, "");
const API = apiUrl('/summary/v2/bcl/dashboard');
...
function renderIndicators(list = []) {
  ...
  return `
    <ul class="ind-list">
      ${list.map(ind => `
        <li>
          <strong>${ind.code ?? ind.id}</strong> â€” ${ind.title ?? 'Untitled'}
...
async function fetchEvidenceDetail(evidenceId) {
  const res = await apiFetch(`/evidence/${encodeURIComponent(evidenceId)}`);
  let json = null;
  try {
    json = await res.json();
  } catch (_) {
...
async function fetchBundle() {
  const res = await apiFetch(`${API}?project_id=07d07ae1-28de-4a12-a342-27c6f052afd4&year=2026&week=10&trend_granularity=month&audit=true`);
...
async function resolveProjectAndPeriodIds(projectCode, year, week) {
  ...
  const rr = await apiFetch(`/projects/${proj.id}/periods`);
...
function renderHeader(header) {
  ...
  el.innerHTML = `<h2>Project ${header.project_id} â€” ${header.period.year} W${header.period.week} ${lockBadge}</h2>`;
}
...
async function fetchProjectIndicators(projectId) {
  ...
  const res = await apiFetch(`/projects/${projectId}/indicators`);
      ...
      <ul class="ind-list exp-list">
        ${rows.map(r => {
          ...
          return `
            <li>
              <div class="exp-row-head">
                <strong>${code}</strong> Ã¢â‚¬â€ ${title}
...
async function fetchIndicators(pid) {
  ...
  const res = await apiFetch(`/indicators?perspective_id=${encodeURIComponent(pid)}`);
  ...
  for (const c of cards) {
    ...
    if (c.items) {
          ...
          ${c.items.map(i => {
            ...
            return `
              <div class="barrow"
                ...
                <div class="tip">
                  ${i.perspective_id} Â· score:${i.score ?? 0} Â· w:${i.weight}% Â· weighted:${v.toFixed(2)}
                </div>
              </div>
            `;
      ...
      if (isAlerts) {
        ...
        div.innerHTML = `
          ...
          <div class="value">
            <span class="badge ${bcls} tooltip">
              ${enabled ? 'Explainable âœ“' : 'Explainability'}
        ...
        // konversi 0â€“100 -> 0â€“5 untuk display metodologis
    ...
    // U2.2c â€” inject indikator ke setiap barrow
      ...
      row.addEventListener('click', async (e) => {
        ...
        openDrawer(
          ...
          `
            ...
            <hr/>
            <p class="hint">Loading indicatorsâ€¦</p>
          `
        );
...
(async () => {
  try {
    ...
    if (CURRENT_PROJECT_ID && header?.period?.year && header?.period?.week) {
      try {
        const rr = await apiFetch(`/projects/${CURRENT_PROJECT_ID}/periods`);
